    ...
    var body: some View {
        VStack(alignment: .leading){
            Button(action: {
                ...
                if self.sheetView == "signup"{
                    SignUpView(self.viewModel)
                } else if self.sheetView == "phone"{
                    PhoneAuth(self.viewModel)
                }
            }
        }
    }
...
struct LoginRow_Previews: PreviewProvider {
    static var previews: some View {
        LoginRow(image: "Icon_Claims_Schwarz", name: "Sign up with email", color: Color.green, viewModel: ViewRouter(), viewRouterName: "", sheetView: "")
    }
}
